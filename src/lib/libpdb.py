...
from numpy import dot, transpose, shape   
from numpy import array as _array 


from string import   digits
from re import split
from math import  log, sin, cos, sqrt, pi, fabs, radians,degrees, acos
from copy import copy 
from .libio import MIO

from .libresidue import RESIDUE as RESI
from .libmath import cross, dihedral_vect,Vector
from .libaa import AACHARGE
...
class PDBCHECK:
    def chkMissingAtoms(self):
        for res_sid in self.res_sids:
            self.resSid2Res(res_sid).chkMissAtom()
            
    def rmResWithMissingAtom(self, vb=0):
        rm_res_sids = []
        for res_sid in self.res_sids:
            rm_res = 0
            if self.resSid2Res(res_sid).miss_atom:
                rm_res_sids.append(res_sid)
        for res_sid in rm_res_sids:
            if vb:
                _res = self.resSid2Res(res_sid)
                print("# delete res_sid= %4d resid= %3d  %s \n" % (res_sid, _res.getResid(), _res.nt_name))
                print("# missing atom: %s " % _res.miss_atoms)
            self.rmRes(res_sid)
        self.num_res = len(self.res_sids)
        self.rm_res_sids = rm_res_sids
        
        
    def rmRes(self, res_sid):
        del self.residues[res_sid]
        self.res_sids.remove(res_sid)        
        
        
    def prnResid(self):
        """ print residue id in pdb file(*.resid) and resid in memory( *.iresid) """
        print("# number of residues = %3d " % self.num_res)
        for res_sid in self.res_sids:
            res = self.resSid2Res(res_sid)
            print("# chain: %s  res_sid= %4d  resid= %4d resn= %4s" % (res.chain_id, res_sid, res.getResid(), res.name))
    
   
            
class RMSD:
    def calculate_rmsd(self, conformation1, conformation2,       
                         is_align = False, is_center = True, vb = False, pdb3p=1):      
        """      
          Calculates the RMSD between two conformations.      
  
        * conformation1: array of dimensions [N,3]       
        * conformation2: array of dimensions [N,3]       
        * is_align: True = modify conformation2 to be 
                   aligned to conformation1      
        * is_center: True = remove average center differences      
                    before calculating RMSD       
                    """      
        #print type(conformation1)
        nrow, ncol = conformation1.shape
        if nrow % 3 == 2:
            conformation1 =conformation1[2:,:]
        nrow, ncol = conformation2.shape
        if nrow % 3 == 2:
            conformation2 = conformation2[2:, :]

        if not shape(conformation1) == shape(conformation2):      
            print("# conformation1 shape: ", conformation1.shape)
            print("# cofnormation2 shape: ", conformation2.shape)
            raise ValueError("calculate_rmsd: conformations not the same size." )     
        elif not len(shape(conformation1)) == 2:      
            raise ValueError("calculate_rmsd: conformations not the correct rank." )     

        vecs1 = conformation1.copy()      
        vecs2 = conformation2.copy()      
        ...
        if is_center:      
            ...
            E0 = sum(sum(vecs1 * vecs1)) + sum(sum(vecs2 * vecs2))      

            correlation_matrix = dot(transpose(vecs2), vecs1)      

            V, S, W_trans = svd(correlation_matrix)      
            ...
            if is_align:      
                ...
                conformation2[:,:] = dot(vecs2, optimal_rotation) + center2    
            rmsd = sqrt(rmsd_sq)
            if vb: print("#  rmsd= %8.4f " %  rmsd) 
            return rmsd     
...
class t_PDBATOM:
    def atomTxt2Atom(self, ln):
        ...
        ln = lstrip(ln)
        atomid = int(ln[6:11])
        atomname= strip(ln[12:16])
        resname = ln[17:20]
        x = float(ln[30:38])
        y = float(ln[38:46])
        z = float(ln[46:54])
        misc = ln[54:72]
        segid = rstrip(ln[72:76])
        chain_id = ln[21]
...
class t_STRUCTRE(MIO, t_PDBATOM):
    ...
    def prnSeq(self, cols=2):
        seq = ""
        for resid in self.resids:
            ln = " %4d %4s" % (resid, self.residues[resid].name)
            seq += self.residues[resid].name
            print(ln) 
        print(seq) 
    
           
    def parseTRJ(self, trj_lns, resi=None, newpdb=True): 
        """ parse pdb text to get residues, sequence, cartesian coordinates ine one trajectory/structure
        """
        residue  = None
        pre_resid = -9999
        pre_resname = None
        ...
        for ln in trj_lns:
            if ln.startswith("ATOM") or ln.startswith("HETATM"):
                ...
                #print "# Warning ter in pdb"
                num_ter += 1
                continue
            elif not ln.startswith("ATOM"):
                print("# Error: unrecognized atom: %s" %  ln)
                continue
        self.addResidue(residue)
        self.chain_ids  = list(self.chain_res_sids.keys())
        self.res_sids =  list(self.residues.keys())
        self.res_sids.sort()
        self.num_res =  len(self.res_sids)
        assert self.num_res ==  self.res_sid
        #print "# num_res: %d  num_ter=%4d" %  (self.num_res, num_ter)
    ...
    def addResidue(self, residue): #v2
        if residue is not None:
            ...
            self.addResid2Tbl(residue.getResid(), self.res_sid)
            self.addResSid2Chain(self.res_sid, residue.getChainID())
            self.residues[self.res_sid] = residue
       
    
    def addResid2Tbl(self, resid, res_sid): #v2
        if resid in self.resid2res_sid_tbl:
            self.resid2res_sid_tbl[resid].append(res_sid)
        else:
            self.resid2res_sid_tbl[resid] = [res_sid]
    ...
    def info(self):
        print("# model_id: %4d  Num_res_in_trj: %4d" %  (self.model_id,  self.num_res))
        
        

class t_NEWPDB(MIO):
    """ accomodate multiple structures or/and multiple chains in one structure """
    def __init__(self):
        self.header =  []
        self.title =  []
        self.compnd =  []
    ...
    def readPDB(self, fp_pdb_gz):
        """ read gziped pdb file   """
        f = gzip.open(fp_pdb_gz, 'rb')
        self.pdb_content =  file_content = f.readlines()
        f.close()
    ...
    def parseModel(self):
        seq_dt =  None
        for mid in self.model_keys:
            mod_obj =  STRUCTRE(mid)
            mod_obj.parseTRJ(self.ens_model_txt[mid])
            mod_obj.info()
            self.model_objs.append(mod_obj)
            if not seq_dt:
                seq_dt =  mod_obj.getSeq()
            #break
        return seq_dt
            
            
    def addRemark(self, ln):
        self.remark.append(ln)
        
        
    def info(self):
        print("# Num_model= %4d" %  self.nummdl)
        print("# num_models_in_pool: %4d" %  self.num_model_in_pool)
        
    
    def parseNUMMDL(self, ln):
        tmp =  ln.split()
        self.nummdl =  int(tmp[1])
        
        
    def prasePDB(self):
        MODEL =  False
        for ln in self.pdb_content:
            ln =  ln.strip()
            if ln.startswith("HEADER"):
                self.header.append(ln)
            elif ln.startswith("TITLE"):
                self.title.append(ln)
            elif ln.startswith("COMPND"):
                self.compnd.append(ln)
            elif ln.startswith("KEYWDS"):
                self.keywds.append(ln)
            elif ln.startswith("EXPDTA"):
                self.expdta.append(ln)
            elif ln.startswith("NUMMDL"):
                self.parseNUMMDL(ln)
            elif ln.startswith("SOURCE"):
                self.source.append(ln)
            elif ln.startswith("AUTHOR"):
                self.author.append(ln)
            elif ln.startswith("REVDAT"):
                self.revdat.append(ln)
            elif ln.startswith("JRNL"):
                self.jrnl.append(ln)
            elif ln.startswith("REMARK"):
                self.addRemark(ln)
            elif ln.startswith("DBREF"):
                self.dbref.append(ln)
            elif ln.startswith("SEQRES"):
                self.seqres.append(ln)
            elif ln.startswith("CRYST"):
                self.cryst.append(ln)
            elif ln.startswith("ORIGX"):
                self.orig.append(ln)
            elif ln.startswith("SCALE"):
                self.scale.append(ln)
            elif ln.startswith("MODEL"):
                MODEL =  True
                tmp =  ln.split()
                model_id =  int(tmp[1])
                model_lns =  []
            elif ln.startswith("ATOM") and not MODEL:
                MODEL =  True
                model_id =  999
                model_lns =  [ln]
            elif ln.startswith("ENDMDL"):
                MODEL =  False
                self.ens_model_txt[model_id] =  model_lns
            elif ln.startswith("TER"):
                MODEL =  False
                self.ens_model_txt[model_id] =  model_lns    
            elif MODEL:
                ...
                if MODEL:
                    MODEL =  False
                    self.ens_model_txt[model_id] =  model_lns
                self.hetatm.append(ln)
            elif ln == "END":
                break
            elif ln.startswith("MASTER"):
                self.master.append(ln)
            else:
                print("# unrecognized text: %s" %  ln)
           
        self.model_keys = list(self.ens_model_txt.keys())
        self.model_keys.sort()
        self.num_model_in_pool =  len(self.model_keys)
        
class PDB(MIO, Vector, PDBCHECK, RMSD): # deprecated
